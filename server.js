...
const CLIENT_ID = process.env.ZOOM_CLIENT_ID || "t3vgM8KeTAWNWfQQOrspBQ";
const CLIENT_SECRET = process.env.ZOOM_CLIENT_SECRET || "5ZpXxcLyvIMQ7t1GiPQhW6SJ88KI2MTT";
const REDIRECT_URI = process.env.REDIRECT_URI || "http://localhost:8000/api/oauth/callback";
...
app.get("/api/auth/zoom", (req, res) => {
  const zoomOAuthUrl = `https://zoom.us/oauth/authorize?response_type=code&client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(
    REDIRECT_URI
  )}`;
...
app.get("/api/meetings/:meetingId/participants/joined", async (req, res) => {
  ...
  try {
    ...
    const joinedParticipants = response.data.participants.filter(
      (participant) => participant.join_time
    );
    res.json({
      message: "Participants who joined",
...
app.get("/api/meetings/:meetingId/participants/left", async (req, res) => {
  ...
  try {
    ...
    const leftParticipants = response.data.participants.filter(
      (participant) => participant.leave_time
    );
    res.json({
      message: "Participants who left",
